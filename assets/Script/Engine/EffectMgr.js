...
EffectMgr.prototype.playEffect = function (effectName, effectNode, effectType) {
    var effect= this.effectPool.get();
    if(null!=effect){
        var effectCtrl = effect.getComponent('EffectCtrl');
        effectCtrl.play(effectNode,effectType);
        return;
    }else{
        this.loadEffectAndPlay(effectName,effectNode,effectType);
    }
};

//加载并播放 功能同上
EffectMgr.prototype.loadEffectAndPlay = function (effectName,effectNode,effectType) {
